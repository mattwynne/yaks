# Goal
Write `plugins/yx/skills/prepare/SKILL.md` — the `/yx:prepare` skill for Claude Code.

# What this skill does
Teaches Claude how to flesh out a yak before implementation —
establishing a spec, acceptance criteria, and breaking work into
sub-yaks.

# Key principles to preserve from the source
- Read existing state before doing anything (Phase 0)
- Mark WIP before starting visible work
- Brainstorm the spec collaboratively with the user
- Store outputs on the yak itself (context, fields)
- Plan as sub-yaks, not as documents — the tree is the plan
- Sub-yak context should be detailed enough for zero-context implementation
- Iron Law: `yx add` then `yx ls` after every addition
- Each phase ends with user confirmation

# Adaptations needed
- Remove all pi-specific references
- Remove example mapping as a prescribed technique — instead encourage
  teams to establish acceptance criteria in their own way
- Encourage capturing acceptance tests (ideally automated) but don't
  mandate it
- Remove references to `/brainstorming`, `/example-mapping` and other
  external skills — make the skill self-contained
- Remove the ADR phase for refactorings (too prescriptive) — just
  encourage capturing the "why" in context
- Use `yx` CLI directly (no custom tools)
- Should work for any Claude Code user with `yx` installed
- Simplify the type system (feature/refactoring/chore) — it adds
  complexity without much value for a general audience
...
- Content covers the preparation workflow
- No pi-specific or external skill references
- Encourages but doesn't mandate specific techniques for acceptance criteria
- A Claude Code agent reading this skill could prepare a yak effectively

# Source material
The following is the existing pi skill this is adapted from.
Preserve the spirit but adapt per the notes above.

---

## Source: .pi/skills/preparing-a-yak/SKILL.md
