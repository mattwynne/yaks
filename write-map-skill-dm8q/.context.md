# Goal
Write `plugins/yx/skills/map/SKILL.md` — the `/yx:map` skill for Claude Code.

# What this skill does
Teaches Claude how to discover and structure work as yaks by
approaching goals and finding blockers — not top-down decomposition.

# Key insight: mapping is deliberately lightweight
Mapping answers "what work exists and what's blocked by what?"
It is NOT detailed planning. The context written during mapping
is just enough to know what a yak is about — a sentence or two,
not a full spec.

For a small thing, mapping might be: one yak, no children, go.
For a big thing, you might spend 20 minutes building a tree
before finding a workable leaf. Either way, the goal is to find
the next thing you can actually do, not to plan everything.

Mapping is distinct from preparing (`/yx:prepare`), which adds
detail to a specific yak before working on it. They're different
activities even if they sometimes flow together in one conversation.

# Key principles to preserve from the source
- "Approach, don't decompose" — discover work by trying to do it
- The Iron Law: `yx add` then `yx ls` after every addition
- Scope conversation before creating children
- Nesting means "blocked by" — children are prerequisites
- Leaf yaks should be small enough to just do
- Growth is bidirectional (down: blockers, up: bigger goals)
- Context during mapping is minimal: what is this yak about?
  (detailed specs come later in `/yx:prepare`)

# Adaptations needed
- Remove all pi-specific references
- Use `yx` CLI directly (no custom tools like start_yak etc.)
- No references to superpowers-dev or other external skills
- Should work for any Claude Code user with `yx` installed
- Keep it self-contained — no dependencies on other skills
- Reference `/yx:prepare` and `/yx:work` as next steps but
  don't depend on them

# Target file
`plugins/yx/skills/map/SKILL.md`

# Definition of Done
- SKILL.md has correct frontmatter (name, description)
- Content covers the mapping workflow
- Mapping is clearly positioned as lightweight structure discovery,
  not detailed planning
- No pi-specific or external skill references
- A Claude Code agent reading this skill could map work effectively

# Source material
The following is the existing pi skill this is adapted from.
Preserve the spirit but adapt for Claude Code + yx CLI.
Pay special attention to keeping mapping lightweight — the source
skill's "Context Pattern" section with full goal/done/knowns/unknowns
is too heavy for mapping. That level of detail belongs in `/yx:prepare`.

---

## Source: .pi/skills/yak-mapping/SKILL.md
