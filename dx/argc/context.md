...
**Recent Progress:**
- ✅ `rm` command fully implemented (5/5 tests passing)
- ✅ `list` enhanced with --format and --only options (working in 16/17 tests)
- ✅ Nested display with indentation (mostly working)
- ✅ Hierarchical yak display (parent/child relationships)
- ⚠️  One edge case remaining: hierarchical sort when child is done needs recursive display approach
...
- ✅ `add` command with validation (11/13 tests) - multi-word names, rejects invalid chars
- ✅ `done` command with --undo, --recursive (6/10 tests) - marks yaks done, checks children
- ✅ `list` command with nested display and filters (16/17 tests):
...
- ✅ `rm` command (5/5 tests) - removes yaks, handles nested yaks, multi-word names
...
1. ✅ **rm** - Remove yaks (5/5 tests COMPLETE)
...
2. 🔄 **Enhance list** - Nested display + filters (16/17 tests, ~95% complete)
   - ✅ Recursive nested display with indentation
   - ✅ --format plain/raw/markdown/md (output full paths or formatted)
   - ✅ --only done/not-done filter
   - ✅ Sorting by priority and mtime within siblings
   - ⚠️  One edge case: hierarchical ordering when mixing done/not-done across levels
...
**Next Steps:**
1. ✅ ~~Implement `rm` command~~ DONE
2. 🔄 Finish list edge case (1 test remaining) - may require recursive display refactor
