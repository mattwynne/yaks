...
Teaches Claude how to pick up ready yaks and implement them — using
git worktrees for isolation, implementing the work, and merging back.
Covers both single yak and parallel (multiple independent yaks).
...
- Mark yaks as WIP before starting
- Merge as soon as tests pass (trunk-based development)
- For parallel: check yaks are independent (don't touch same files)
- Never modify main's working tree directly
- Always review agent output before marking done

# Adaptations needed
- Remove all pi-specific tools (start_yak, merge_yak, show_yak, subagent)
...
- For parallel work, use Claude Code's native subagent/background
  agent capabilities instead of pi-specific delegation
