Describe 'yx sync'
  ...
  It 'pushes yaks to origin'
    ...
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1
  ...
  It 'pulls yaks from origin'
    ...
    GIT_PATH="$USER1" "yx" add "shared yak"
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1

    # User2 syncs and should get the yak
    sh -c "cd '$USER2' && GIT_PATH='$USER2' yx sync" 2>&1

    When call sh -c "GIT_PATH='$USER2' yx ls"
    The output should include "shared yak"
  End

  It 'merges yaks from multiple users'
    # User1 adds a yak
    GIT_PATH="$USER1" "yx" add "user1 yak"
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1

    # User2 adds a different yak
    GIT_PATH="$USER2" "yx" add "user2 yak"
    sh -c "cd '$USER2' && GIT_PATH='$USER2' yx sync" 2>&1

    # User1 syncs again and should have both
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1

    When call sh -c "GIT_PATH='$USER1' yx ls"
  ...
  It 'handles concurrent modifications to same yak'
    ...
    GIT_PATH="$USER1" "yx" done "shared yak"

    # User2 adds context to same yak (without syncing yet)
    echo "User2 context" | GIT_PATH="$USER2" "yx" context "shared yak"

    # User1 syncs first
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1

    # User2 syncs - should trigger merge
    When call sh -c "cd '$USER2' && GIT_PATH='$USER2' yx sync 2>&1"
    The status should be success
  End

  It 'resolves concurrent edits with last-write-wins'
    # User1 adds a yak and syncs
    GIT_PATH="$USER1" "yx" add "conflict yak"
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1
    ...
    GIT_PATH="$USER1" "yx" done "conflict yak"
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1

    # User2 also marks done (creating same state)
    GIT_PATH="$USER2" "yx" done "conflict yak"

    # User2 syncs - should handle duplicate done gracefully
    sh -c "cd '$USER2' && GIT_PATH='$USER2' yx sync" 2>&1

    # Both should show done
    result1=$(sh -c "GIT_PATH='$USER1' yx ls")
    result2=$(sh -c "GIT_PATH='$USER2' yx ls")
  ...
  It 'does not restore pruned yaks after sync with divergent remote'
    ...
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx add 'done yak'"
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx done 'done yak'"
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1
    ...
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx prune"

    # Meanwhile, User2 adds a different yak and syncs (creates divergence)
    sh -c "cd '$USER2' && GIT_PATH='$USER2' yx add 'user2 yak'"
    sh -c "cd '$USER2' && GIT_PATH='$USER2' yx sync" 2>&1

    # Now User1 syncs - this should merge but NOT bring back done yak
    sh -c "cd '$USER1' && GIT_PATH='$USER1' yx sync" 2>&1

    When call sh -c "cd '$USER1' && GIT_PATH='$USER1' yx ls"
